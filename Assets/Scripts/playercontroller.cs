...
public class playercontroller : MonoBehaviour
{
    ...
    private void Start()
    {
        mCamera = GameObject.Find("Orbit Camera");
    }
    ...
    void Update()
    {
        ...
        if (skill)
        {
            ...
            {
                TargetPosition = transform.position + 12 * mCamera.transform.forward;
               // GameObject.Instantiate(bullet, transform.position, Quaternion.identity);//生成子弹
                timeToggle = !timeToggle;
                Time.timeScale = timeToggle ? TimeScale : defaultTimeScale;
                Time.fixedDeltaTime = defaultfixedDeltaTime * Time.timeScale;
                //Ray CameraRay = Camera.main.ScreenPointToRay(Input.mousePosition);
                //Ray CameraRay = new Ray(transform.position, mCamera.transform.forward);
                //Debug.DrawRay(transform.position,10*mCamera.transform.forward, Color.green);
                //RaycastHit hit1;
            ...
            {
            time = 0;
            Ray ray = new Ray(transform.position, -mCamera.transform.right);
                RaycastHit hit;
                ...
                if (!ishit)
                {
                    TargetPosition = transform.position - 4 * mCamera.transform.right;
                }
                //TargetPosition = transform.position - 5*mCamera.transform.right;
            ...
            {
            time = 0;
            Ray ray = new Ray(transform.position, mCamera.transform.forward);
                RaycastHit hit;
                bool ishit = Physics.Raycast(ray, out hit, 4f);
                if (ishit)
                {
                ...
                if (!ishit)
                {
                    TargetPosition = transform.position + 4 * mCamera.transform.forward;
                }
                //TargetPosition = transform.position + 5 * mCamera.transform.forward;
                transform.position = new Vector3(Mathf.SmoothStep(transform.position.x, TargetPosition.x, 1f), Mathf.SmoothStep(transform.position.y,
                    TargetPosition.y, 1f), Mathf.SmoothStep(transform.position.z, TargetPosition.z, 1f));
                Invoke("timescale", 0.02f);
            }
